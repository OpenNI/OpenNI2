...
{
PerCDepthStream::PerCDepthStream(XnUInt32 idxDeviceInternal, XnUInt32 idxStream, const OniVideoMode &videoMode)
    : PerCBaseStream(idxDeviceInternal, idxStream, videoMode)
{
    if (!m_pxcDevice.IsValid())
        return;

    PXCSession *session = deviceEnumerator().getSession();
    if (NULL == session)
        return;

    //PXCSessionService *service = session->DynamicCast<PXCSessionSessionService>();
    //if (NULL != service)
    //{
    //    if (PXC_STATUS_NO_ERROR == service->LockSession())
    //    {
    //        m_pxcDevice->SetProperty(PXCCapture::Device::PROPERTY_DEPTH_LOW_CONFIDENCE_VALUE, (pxcF32)-1);
    //        m_pxcDevice->SetProperty(PXCCapture::Device::PROPERTY_DEPTH_SATURATION_VALUE, (pxcF32)-1);
    //        service->UnlockSession();
    //    }
    //}    

    pxcUID pid = 0;
    m_pxcDevice->QueryPropertyAsUID(PXCCapture::Device::PROPERTY_PROJECTION_SERIALIZABLE, &pid); 
    session->DynamicCast<PXCMetadata>()->CreateSerializable<PXCProjection>(pid, &m_pxcProjection);
}

OniStatus PerCDepthStream::SetVideoMode(OniVideoMode* pVideoMode)
{
    if (m_running)
...
OniStatus PerCDepthStream::GetVideoMode(OniVideoMode* pVideoMode)
{
...
OniStatus PerCDepthStream::convertDepthToColorCoordinates(StreamBase* colorStream, int depthX, int depthY, OniDepthPixel depthZ, int* pColorX, int* pColorY)
{
    if (!m_pxcProjection.IsValid())
        return ONI_STATUS_NOT_SUPPORTED;
    if ((0 == m_videoMode.resolutionX) || (0 == m_videoMode.resolutionY))
    ...
    PXCPoint3DF32 posd = {(float)depthX, (float)depthY, (float)depthZ};
    PXCPointF32   posc;
    m_pxcProjection->MapDepthToColorCoordinates(1, &posd, &posc); 

    *pColorX = (int)(posc.x * (float)videoMode.resolutionX / (float)m_videoMode.resolutionX);
    *pColorY = (int)(posc.y * (float)videoMode.resolutionY / (float)m_videoMode.resolutionY);

    return ONI_STATUS_OK;
}

void PerCDepthStream::Mainloop()
{
    if (NULL == m_pxcStream)
    {
        m_running = false;
    ...
    while (m_running)
    {
        ...
        if (m_mirror)
        {
            ...
            {
                pxcI16 *pSrc = (pxcI16 *)(data.planes[0] + data.pitches[0] * (row + top) + sizeof(short) * (m_videoMode.resolutionX - 1 - left));
        ...
        else
        {
            for (int row = 0; row < pFrame->height; row++)
            {
                pxcI16 *pSrc = (pxcI16 *)(data.planes[0] + data.pitches[0] * (row + top) + sizeof(short) * left);
                pxcI16 *pDst = (pxcI16 *)((pxcBYTE *)(pFrame->data) + pFrame->stride * row);
