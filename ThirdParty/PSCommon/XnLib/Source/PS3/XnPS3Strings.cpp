...
// Globals
//---------------------------------------------------------------------------
static const unsigned long XnOSStrCRC32Table[0x100] =
{
...
XN_CORE_API XnStatus XnOSStrFormat(XN_CHAR* cpDestString, const XN_UINT32 nDestLength, XN_UINT32* pnBytesWritten, const XN_CHAR* cpFormat, ...)
{
	// Validate the input/output pointers (to make sure none of them is NULL)
	XN_VALIDATE_INPUT_PTR(cpDestString);
	...
	*pnBytesWritten = 0;
	...
	XN_INT32 nRes = vsnprintf(cpDestString, nDestLength, cpFormat, args);
	...
	// return byte count (string length + NULL termination)
	*pnBytesWritten = nRes + 1;
	va_end(args);
...
{
	static XN_CHAR cNum[] = "0123456789abcdefghijklmnopqrstuvwxyz";
	XN_CHAR* cpWStr = cpStr;
	XN_INT32 nSign;
	...
	*cpWStr='\0';
	...
	// Reverse string
	XN_CHAR cAux;
	XN_CHAR* cpBegin = cpStr;
	XN_CHAR* cpEnd = cpWStr-1;
